...
const Investigadores = () => {
  ...
  const [newInvestigador, setNewInvestigador] = useState(initialInvestigadorState);
    ...
    const fetchInitialData = async () => {
      try {
        const [investigadoresRes, nivelesRes, areasRes] = await Promise.all([
          ...
          axios.get(`${API_BASE_URL}api/nivel-edu-api/`, {  
          }).catch(e => ({ data: [] })),
          axios.get(`${API_BASE_URL}api/areas-api/`, {  
            withCredentials: true,
          }).catch(e => ({ data: [] }))
        ]);
    ...
    try {
      ...
      setInvestigadores(
        investigadores.map((i) =>
          i.idinvestigadores === editingInvestigador.idinvestigadores ? response.data : i
        )
      );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setInvestigadores(investigadores.filter((i) => i.idinvestigadores !== id));
      setError(null);
    } catch (error) {
      setError(
            ...
            <tbody>
              {investigadores.map((investigador) => (
                <tr key={investigador.idinvestigadores}>
                  ...
                  <td>
                    {options.areas.find(
                      (a) => a.idarea === investigador.idarea
                    )?.nombrearea || "-"}
                  </td>
                  <td>
                    <button
                    ...
                    <button
                      ...
                      onClick={() => handleDelete(investigador.idinvestigadores)}
                    >
                      Eliminar
                    </button>
                  </td>
